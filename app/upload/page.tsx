...
export default function UploadCardPage() {
  ...
  useEffect(() => {
    if (!form.category1) { 
      setSubCats([]); setForm(f => ({ ...f, category2: "" }));
      setThirdCats([]); setForm(f => ({ ...f, category3: "" }));
      return; 
    }
    setSubCats(categories.filter(row => row.level === 2 && row.parentid == form.category1));
    setForm(f => ({ ...f, category2: "" }));
    setThirdCats([]); setForm(f => ({ ...f, category3: "" }));
  }, [form.category1, categories]);
  ...
  useEffect(() => {
    ...
    setThirdCats(categories.filter(row => row.level === 3 && row.parentid == form.category2));
    setForm(f => ({ ...f, category3: "" }));
  }, [form.category2, categories]);
