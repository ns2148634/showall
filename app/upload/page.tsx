...
export default function UploadCardPage() {
  ...
  async function handleSubmit(e) {
    ...
    setMsg("")
    // 前端驗證
    if (!form.email.match(/.+@.+\..+/) || form.email.length > 120) { setMsg("請填正確電子郵件（最多120字）"); return }
    if (form.company.length > 30) { setMsg("公司/組織最多30字"); return }
    if (form.name.length > 20) { setMsg("姓名/暱稱最多20字"); return }
    if (form.line.length > 30) { setMsg("Line最多30字"); return }
    if (form.mobile.length > 30) { setMsg("手機最多30字"); return }
    if (form.contact_other.length > 30) { setMsg("其他聯絡最多30字"); return }
    if (form.intro.length > 30) { setMsg("簡介最多30字"); return }
    if (!form.image_url_front && !imgFront && !previewFront) { setMsg("需上傳名片正面！"); return }
    setLoading(true)
    let image_url_front = previewFront
    let image_url_back = previewBack
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-lg mx-auto py-10">
        <form className="space-y-4 bg-white p-6 rounded-lg shadow" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="font-bold text-gray-600 mb-1 block">電子郵件<span className="text-red-500">*</span> (限120字)</label>
            <input type="email" className="border p-2 rounded w-full" required value={form.email} maxLength={120} onChange={e=>setForm(f=>({...f,email:e.target.value}))} />
          </div>
          <div>
            <label className="font-bold text-gray-600 mb-1 block">公司名稱／組織名稱 (限30字)</label>
            <input type="text" className="border p-2 rounded w-full" value={form.company} maxLength={30} onChange={e=>setForm(f=>({...f,company:e.target.value}))} />
          </div>
          <div>
            <label className="font-bold text-gray-600 mb-1 block">姓名／暱稱 (限20字)</label>
            <input type="text" className="border p-2 rounded w-full" value={form.name} maxLength={20} onChange={e=>setForm(f=>({...f,name:e.target.value}))} />
          </div>
          {/* 三層分類 */}
          ...
          <div>
            ...
            <input type="text" className="border p-2 rounded w-full" value={form.line} maxLength={30} onChange={e=>setForm(f=>({...f,line:e.target.value}))} />
          </div>
          <div>
            <label className="font-bold text-gray-600 mb-1 block">手機 (限15字)</label>
            <input type="text" className="border p-2 rounded w-full" value={form.mobile} maxLength={30} onChange={e=>setForm(f=>({...f,mobile:e.target.value}))} />
          </div>
          <div>
            <label className="font-bold text-gray-600 mb-1 block">其他連絡方式 (限30字)</label>
            <input type="text" className="border p-2 rounded w-full" value={form.contact_other} maxLength={30} onChange={e=>setForm(f=>({...f,contact_other:e.target.value}))} />
          </div>
          <div>
            <label className="font-bold text-gray-600 mb-1 block">自我簡介（限30字）</label>
            <input type="text" maxLength={30} className="border p-2 rounded w-full" value={form.intro} onChange={e=>setForm(f=>({...f,intro:e.target.value}))} />
          </div>
          <div>
